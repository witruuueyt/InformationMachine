...
namespace game4automation
{
    ...
    public class SelectionWindow : EditorWindow
    {
        ...
        {
            settings = UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/game4automation/private/ProductiveTools/Editor/SelectionWindowSettings.asset", typeof(SelectionWindowSettings)) as SelectionWindowSettings;
            if (settings != null)
            {
        ...
        [MenuItem("game4automation/Add to selection", false, 401)]
        ...
        [MenuItem("game4automation/Remove from selection", false, 402)]
        ...
        private void UnhideAll()
        {
            ...
            {
                if (obj.GetComponents<Game4AutomationController>().Length == 0)
                {
        ...
        private void RemoveG4A()
        {
            ...
            foreach (var obj in objs)
            {
                ...
                var behaviors = obj.GetComponents<Game4AutomationBehavior>().ToArray();
        ...
        private void AssignShader(string shader)
        {
            if (shader == "null")
            {
                EditorUtility.DisplayDialog("No new shaderfor asignment selected", "Please select a new material", "OK");
                return;
            }
            if (EditorUtility.DisplayCancelableProgressBar("Collecting objects", "Please wait",
                0))
            ...
            foreach (var myobj in list)
            {
                ...
                if (renderer != null)
                {
                    if (EditorUtility.DisplayCancelableProgressBar("Progressing objects", $"Shader update on object {a} of {list.Count}",
                        progress))
                    {
                        EditorUtility.ClearProgressBar();
                        return;
        ...
        private void DoMaterialUpdate(MaterialUpdateSettings materialupdate)
        {
            ...
            {
                EditorUtility.DisplayDialog("No Material Update Definiton", "Please select a material update definition", "OK");
                return;
            }
            if (EditorUtility.DisplayCancelableProgressBar("Collecting objects", "Please wait",
                0))
            ...
            foreach (var myobj in list)
            {
                ...
                if (EditorUtility.DisplayCancelableProgressBar("Progressing objects", $"Material update on object {i} of {list.Count}",
                    progress))
                {
                    EditorUtility.ClearProgressBar();
                    return;
        ...
        [MenuItem("game4automation/Selection window (Pro)", false, 400)]
        static void Init()
        {
        ...
        void OnGUI()
        {
          ...
          EditorGUILayout.BeginHorizontal();
                ...
                selectedmaterial2 = (Material) EditorGUILayout.ObjectField(selectedmaterial2, typeof(Material), false,GUILayout.Width((width / 2)-15));
                EditorGUILayout.EndHorizontal();
                ...
                selectedassingmaterial = (Material) EditorGUILayout.ObjectField(selectedassingmaterial, typeof(Material), false,GUILayout.Width((width / 2)-15));
                ...
                selectedmaterialupdate = (MaterialUpdateSettings) EditorGUILayout.ObjectField(selectedmaterialupdate, typeof(MaterialUpdateSettings), false,GUILayout.Width((width / 2)-15));
                ...
                selectedtonewparent = EditorGUILayout.ObjectField(selectedtonewparent, typeof(GameObject), true,GUILayout.Width((width / 2)-15));
            ...
            if (GUILayout.Button("Rename", GUILayout.Width((width / 2))))
            {
                RenameGroup();
            }
            ...
            if (!condensedgroupview)
            {
                ...
                selectedpivottoorigin = EditorGUILayout.ObjectField(selectedpivottoorigin, typeof(GameObject), true);
                if (GUILayout.Button("Set", GUILayout.Width((width / 5) - 10)))
                {
                    EditorGUIUtility.PingObject(Selection.activeObject);
